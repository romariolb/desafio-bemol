<template>
  <div class="q-pa-md">
    <q-card-section>
      <div class="row">
        <div class="col-6">
          <Titulo titulo="Editar Usuário" subtitulo="Editar Usuário" />
        </div>
      </div>
      <ContainerForm>
...
export default {
  ...
  watch: {
    ...
    getUser(){
      ...
      if (this.user.cpf != null) {
        this.user.tipo_doc = 0
        this.cpf = this.user.cpf
        this.listaSexo_sel = {
          label: this.user.sexo,
          value: this.user.sexo
        }
      } else {
        this.user.tipo_doc = 1
  ...
  computed: {
    ...mapState('User', ['updating']),
    ...mapState('Endereco', ['updating']),

    ...mapGetters('User', ['getUser']),
  },
  methods: {

    ...mapActions('User', ['loadUser', 'atualizaUser']),
    ...mapActions('Endereco', ['loadEndereco', 'atualizaEndereco']),
    ...
    async save () {
      if (this.user.tipo_doc == 0) {
        if (this.validarCampos1() == false && this.validarCampos3() == false) {
          ...
          const result2 = await this.atualizaEndereco(this.endereco)
            if (result2.status == true) {
              ...
              const result = await this.atualizaUser(this.user)
